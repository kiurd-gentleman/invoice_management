        ...
        <table class="table mb-0 thead-border-top-0 table-striped">
            ...
            <tbody class="list" id="plans">
                @foreach ($plans as $plan)
                    <tr>
                        <td>
                            <a href="{{ route('super_admin.plans.edit', $plan->id) }}" class="badge badge-info">#{{ $plan->id }}</a>
                        </td>
                        <td>
                        ...
                        <td class="text-center"><i class="ft-calendar text-muted-light mr-1"></i> {{ $plan->created_at->format('Y-m-d') }}</td>
                        <td><a href="{{ route('super_admin.plans.edit', $plan->id) }}" class="btn btn-sm btn-link"><i class="ft-arrow-right"></i></a></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
